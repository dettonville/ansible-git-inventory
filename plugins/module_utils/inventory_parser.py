...
_CLASS_DEFAULT_VALUES = {
    ...
    "global_groups_file": "global_groups.yml",
...
class InventoryParser:
    ...
    def __init__(
        ...
        inventory_repo_dir=_CLASS_DEFAULT_VALUES["inventory_repo_dir"],
        ...
        if inventory_repo_dir:
            self.inventory_repo_dir = inventory_repo_dir
        else:
            self.inventory_repo_dir = tempfile.mkdtemp(prefix="update_inventory")

        # self.inventory_dir = os.path.dirname(self.inventory_file_path)
        if inventory_dir:
            self.inventory_dir = os.path.join(self.inventory_repo_dir, inventory_dir)
            self.inventory_file_path = os.path.join(
                self.inventory_repo_dir, self.inventory_dir, self.inventory_file
            )
        else:
            self.inventory_file_path = os.path.join(
                self.inventory_repo_dir, self.inventory_file
            )
            self.inventory_dir = os.path.dirname(self.inventory_file_path)
        ...
        self.log.info(
            "%s inventory_repo_dir => %s", log_prefix, self.inventory_repo_dir
        )
        self.log.debug("%s inventory_dir => %s", log_prefix, self.inventory_dir)
        self.log.debug(
        ...
        if not os.path.exists(self.inventory_file_path):
            ...
            raise InventoryParserException(
                "%s inventory file %s does not exist" % (log_prefix, self.inventory_file_path))
        ...
        self.inventory_yaml = self.yaml_parser.load_from_file(self.inventory_file_path)
        self.log.debug(
            "%s self.inventory_yaml=%s",
            log_prefix,
            self.inventory_yaml,
        )

        self.inventory = self.inventory_yaml[self.inventory_root_yaml_key]
        ...
        if self.enforce_global_groups_must_already_exist:
            ...
            if not os.path.exists(self.global_groups_path):
                ...
                raise InventoryParserException(
                    "%s file does not exist at global_groups_path=%s" % (log_prefix, self.global_groups_path))
    ...
    def update_yaml_file(self, filepath, yaml_content):
        ...
        if self.backup:
            ...
            self.backup_files.extend([backup_file.replace(self.inventory_repo_dir, "")])
        ...
        try:
            ...
            raise InventoryParserException(
                "%s write temporary file at %s results in IOError => %s" % (log_prefix, tmpfile, e))
        except Exception as e:
            raise InventoryParserException(
                "%s write temporary file at %s results in error => %s" % (log_prefix, tmpfile, e),
        ...
        try:
            ...
            raise InventoryParserException(
                "%s move temporary file %s to %s results in IOError => %s" % (log_prefix, tmpfile, filepath, e))
        except Exception as e:
            raise InventoryParserException(
                "%s move temporary file %s to %s results in error => %s" % (log_prefix, tmpfile, filepath, e),
                traceback=traceback.format_exc())
        finally:
        ...
        if YAMLLINT_IMPORT_ERROR:
            ...
            self.log.warning(
                "%s missing yamllint - linting will be skipped for => %s",
                log_prefix,
                filepath,
            )
            return
        ...
        try:
            ...
            if yaml_lint_errors:
                raise InventoryParserException(
                    "%s yamllint errors on file [%s] occurred: %s" % (log_prefix, filepath, to_text(yaml_lint_errors)))

        except SystemExit as yaml_lint_excp:
            # yaml_lint_errors |= bool(e.code)
            raise InventoryParserException(
                "%s yamllint exception on file [%s] occurred: %s " % (log_prefix, filepath, to_text(yaml_lint_excp)))

        return result

    # def validate_inventory_repo(self):
    #     log_prefix = "%s.validate_inventory_repo(%s):" % (self.__class__.__name__)
    #     result = dict(
        ...
        if not os.path.exists(self.groupvars_dir):
            try:
                ...
                if not (e.errno == errno.EEXIST and os.path.isdir(self.groupvars_dir)):
                    raise InventoryParserException(
                        "%s mkdir groupvars_dir at %s results in error => %s" % (log_prefix, self.groupvars_dir, e))
    ...
    def validate_global_group_exists(self, group_name):
        ...
        if not key_value_list:
            ...
            raise InventoryParserException(
                "%s group_name=[%s] not found in %s" % (log_prefix, group_name, self.global_groups_file))
        ...
        if not os.path.exists(self.hostvars_dir):
            try:
                ...
                if not (e.errno == errno.EEXIST and os.path.isdir(self.hostvars_dir)):
                    raise InventoryParserException(
                        "%s create groupvars_dir at %s results in error => %s" % (log_prefix, self.groupvars_dir, e))
    ...
    def save_inventory(self):
        ...
        result.update(
            self.update_yaml_file(self.inventory_file_path, self.inventory_yaml)
        )
        if self.backup:
