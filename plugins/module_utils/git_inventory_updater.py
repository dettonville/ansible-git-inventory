...
def class_init_wrapper(cls: Type[T], *args, **kwargs) -> T:
    ...
    for name, param in init_signature.parameters.items():
        ...
        if param.kind == inspect.Parameter.VAR_KEYWORD:
            ...
            remaining_kwargs = {k: v for k, v in kwargs.items() if k not in valid_params}
    ...
    if missing_required:
        raise TypeError(f"Missing required parameters for {cls.__name__}: {missing_required}")
...
class GitInventoryUpdater:
    ...
    def __init__(self, module, git_repo_config=None, test_mode=False, remove_repo_dir=True, **kwargs):
        ...
        self.collection_version = get_collection_version(
            [self.module_fqcn], no_version=None
        )[0]
        self.log.debug("%s collection_version=%s", log_prefix, self.collection_version)

        self.remove_repo_dir = remove_repo_dir
        self.test_mode = test_mode

        if kwargs["inventory_repo_dir"] is None:
            kwargs["inventory_repo_dir"] = tempfile.mkdtemp(prefix="update_inventory")

        self.inventory_repo_dir = kwargs["inventory_repo_dir"]
        self.log.debug("%s inventory_repo_dir => %s", log_prefix, self.inventory_repo_dir)
        ...
        self.inventory_parser = class_init_wrapper(InventoryParser, self.module, **kwargs)
    ...
    def load_git_repo(self, **kwargs):
        ...
        self.git_repo_config["repo_dir"] = self.inventory_repo_dir
        self.git_comment_prefix = kwargs["git_comment_prefix"]
        self.git_comment_body = kwargs["git_comment_body"]
        ...
        self.log.debug("%s cloning repo", log_prefix)
    ...
    def update_inventory(self, group_list=None, host_list=None, state="merge"):
        ...
        if self.git_repo_config:
            git_repo_update = self.update_git_repo()
            result.update(git_repo_update)
    ...
    def update_git_repo(self):
        ...
        if self.remove_repo_dir:
            try:
                shutil.rmtree(self.inventory_repo_dir)
            except OSError as e:
            ...
            result["inventory_repo_dir"] = self.inventory_repo_dir
