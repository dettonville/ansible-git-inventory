...
class GitInventoryUpdater:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.inventory_base_dir = kwargs.get(
            "inventory_base_dir", tempfile.mkdtemp(prefix="update_inventory_")
        )
        self.inventory_base_dir = self.inventory_base_dir
    ...
    def get_internal_collection_version(self):
        ...
        try:
            collection_version_full = get_collection_version(
                [module_fqcn], no_version=None
            )
            self.log.debug(
                "%s collection_version_full=%s", log_prefix, collection_version_full
            )
            collection_version = collection_version_full[0]
