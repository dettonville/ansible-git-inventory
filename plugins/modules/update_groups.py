...
DOCUMENTATION = r"""
---
module: update_groups
author:
    - "Lee Johnson (@lj020326)"
short_description: Add/update group nodes and respective variable settings to a specified YAML-file based inventory. 
description:
  Ansible module to add, update, and/or remove group nodes to a specified YAML-file
...
options:
    ...
    use_vars_files:
        description:
            - Use vars files ('group_vars/') for group vars instead of inline group_vars.
        type: bool
        default: true
    create_empty_groupvars_files:
        description:
            - Creates empty vars files ('group_vars/' or 'host_vars/') for group/host vars even when no vars specified.
...
argument_spec = dict(
    ...
    inventory_repo_dir=dict(required=False, type="path"),
    ...
    group_list=dict(required=True, aliases=["groups"], type="list", elements="dict"),
...
def run_module():
    ...
    inventory_repo_dir = module.params.get("inventory_repo_dir") or None
    inventory_dir = module.params.get("inventory_dir") or None
    inventory_file = module.params.get("inventory_file")
    inventory_root_yaml_key = module.params.get("inventory_root_yaml_key")
    global_groups_file = module.params.get("global_groups_file")

    remove_repo_dir = module.params.get("remove_repo_dir")
    test_mode = module.params.get("test_mode")

    git_comment_prefix = module.params.get("git_comment_prefix")
    # logging.info("git_comment_prefix => %s", git_comment_prefix)
    git_comment_body = module.params.get("git_comment_body")
    ...
    vars_state = module.params.get("vars_state")
    vars_overwrite_depth = module.params.get("vars_overwrite_depth")
    use_vars_files = module.params.get("use_vars_files")
    create_empty_groupvars_files = module.params.get("create_empty_groupvars_files")
    create_parent_groupvar_files = module.params.get("create_parent_groupvar_files")
    always_add_child_group_to_root = module.params.get("always_add_child_group_to_root")
    enforce_global_groups_must_already_exist = module.params.get(
        "enforce_global_groups_must_already_exist"
    )
    enable_groupvar_symlinks_for_child_inventories = module.params.get(
        "enable_groupvar_symlinks_for_child_inventories"
    )
    symlink_subdirs = module.params.get("symlink_subdirs")
    ...
    inventory_repo = GitInventoryUpdater(
        module,
        git_repo_config,
        inventory_file=inventory_file,
        yaml_lib_mode=yaml_lib_mode,
        inventory_root_yaml_key=inventory_root_yaml_key,
        global_groups_file=global_groups_file,
        inventory_repo_dir=inventory_repo_dir,
        inventory_dir=inventory_dir,
        vars_state=vars_state,
        vars_overwrite_depth=vars_overwrite_depth,
        use_vars_files=use_vars_files,
        create_empty_groupvars_files=create_empty_groupvars_files,
        create_parent_groupvar_files=create_parent_groupvar_files,
        always_add_child_group_to_root=always_add_child_group_to_root,
        enforce_global_groups_must_already_exist=enforce_global_groups_must_already_exist,
        enable_groupvar_symlinks_for_child_inventories=enable_groupvar_symlinks_for_child_inventories,
        symlink_subdirs=symlink_subdirs,
        git_comment_prefix=git_comment_prefix,
        git_comment_body=git_comment_body,
        backup=backup,
        test_mode=test_mode,
        remove_repo_dir=remove_repo_dir,
    )

    update_result = inventory_repo.update_inventory(
