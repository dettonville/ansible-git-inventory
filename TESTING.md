...
```bash
pytest tests/unit/plugins/modules/test_update_inventory.py
```
...
./run-tests.sh units tests/unit/plugins/modules/test_update_inventory.py
```
...
```bash
ln -s "$(pwd)" ~/.ansible/collections/ansible_collections/dettonville/git_inventory
```

#### Step 3: Navigate to the symlinked path and run tests
```bash
cd ~/.ansible/collections/ansible_collections/dettonville/git_inventory
```
...
ansible-test units -v --python 3.13 update_inventory
...
```bash
cd path/to/dettonville.git_inventory
```
...
| **Direct `ansible-test`** | Symlink Directory (`~/.ansible/.../git_inventory`) | **One-time manual symlink** setup             | `ansible-test units --python 3.13 [module]` |
